...
static AstStmtNodePtr GlobalDeclStatement(void) {
    if (IS_PREFIX_OF_DECL(curToken.kind)) {
        ...
        Error(__FILE__, __LINE__, "'long' expected");
        return NULL;
    }
}
...
static AstStmtNodePtr ExpressionStatement(void) {
    if (curToken.kind == TK_ID) {
        ...
        if (curToken.kind == TK_ASSIGN) {
            ...
            assign->kids[0] =
                CreateAstExprNode(TK_ID, &savedToken.value, NULL, NULL);
            NEXT_TOKEN;
            ...
            Error(__FILE__, __LINE__, "'=' expected");
        }
        Expect(TK_SEMICOLON);
        return res_stmt;
    }
        ...
        Error(__FILE__, __LINE__, "'id' or 'long' expected");
        return NULL;
    }
}
...
AstStmtNodePtr IOStatement(TokenKind tk) {
    ...
    if (curToken.kind == TK_ID) {
        ioStmt->expr =
            CreateAstExprNode(curToken.kind, &curToken.value, NULL, NULL);
        NEXT_TOKEN;
    } else {
        Error(__FILE__, __LINE__, "'id' expected");
    }
    Expect(TK_RPAREN);
    Expect(TK_SEMICOLON);
    return ioStmt;
...
void EmitStatementNode(AstStmtNodePtr stmt) {
    ...
    case TK_IF:
        ...
        if (stmt->kids[0] && stmt->kids[1]) {
            ...
            EmitComments("\n");
            EmitIR("# start of if(%s) {} else {}", GetNodeNameInIR(stmt->expr));
            EmitIR("# if(!%s) goto %s", GetNodeNameInIR(stmt->expr),
            ...
            EmitIR("# end of if(%s) {} else {}", GetNodeNameInIR(stmt->expr));
            EmitLabel("%s:", stmt->kids[1]->value.name);
        } else {
            EmitComments("\n");
            EmitIR("# start of if(%s) {}", GetNodeNameInIR(stmt->expr));
            EmitIR("# if(!%s) goto %s", GetNodeNameInIR(stmt->expr),
                   stmt->kids[0]->value.name);
            EmitAstExprNode(stmt->expr);
            EmitAssembly("cmpq $0, %s", GetNodeNameInAssembly(stmt->expr));
            EmitAssembly("je %s", stmt->kids[0]->value.name);
            EmitStatementNode(stmt->thenStmt);
            EmitIR("# end of if(%s) {}", GetNodeNameInIR(stmt->expr));
            EmitLabel("%s:", stmt->kids[0]->value.name);
        }
        break;
    case TK_DO:
        ...
        EmitComments("\n");
        EmitIR("# end of do {} while(%s)", GetNodeNameInIR(stmt->expr));
        EmitLabel("%s:", stmt->kids[1]->value.name);
        break;
    case TK_WHILE:
        /*****************************************************
        ...
        EmitComments("\n");
        EmitIR("# end of while(%s) do {}", GetNodeNameInIR(stmt->expr));
        EmitLabel("%s:", stmt->kids[1]->value.name);
    ...
    case TK_ASSIGN: // id = expression;
        ...
        if (stmt->kids[0] && stmt->expr) {
            EmitIR("# %s = %s;", GetNodeNameInIR(stmt->kids[0]),
                   GetNodeNameInIR(stmt->expr));
            EmitAssembly("movq %s, %%rax", GetNodeNameInAssembly(stmt->expr));
            EmitAssembly("movq %%rax, %s",
                         GetNodeNameInAssembly(stmt->kids[0]));
        }
        break;
    ...
    case TK_RETURN: // return expression;
        ...
        EmitComments(
            "# Save current function's return value %s in register rax",
            GetNodeNameInIR(stmt->expr));
        EmitAssembly("movq %s, %%rax", GetNodeNameInAssembly(stmt->expr));
        break;
    default:
